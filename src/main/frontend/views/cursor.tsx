import { MouseEvent, memo } from "react";
import { ValueSignal } from "@vaadin/hilla-react-signals";
import type Position from "Frontend/generated/com/github/taefi/services/CursorService/Position.js";
import { VerticalLayout } from "@vaadin/react-components/VerticalLayout";
import { CursorService } from "Frontend/generated/endpoints.js";
import { ViewConfig } from "@vaadin/hilla-file-router/types.js";

export const config: ViewConfig = {
  menu: { order: 3, icon: 'vaadin:cursor' },
  title: 'Shared Cursor',
};

function getRelativePosition(event: MouseEvent): Position {
  const rect = event.currentTarget.getBoundingClientRect();
  return { x: event.clientX - rect.left, y: event.clientY - rect.top};
}

const sharedCursor: ValueSignal<Position | undefined> = CursorService.sharedCursor({defaultValue: {x: 0, y: 0}});

export default function Cursors() {

  function move(event: MouseEvent): void {
    if (event.currentTarget.tagName != "DIV") return;
    sharedCursor.value = getRelativePosition(event);
  }

  function leave(event: MouseEvent): void {
    if (sharedCursor.value) {
      sharedCursor.value = undefined;
    }
  }

  return (
    <VerticalLayout>
      <div style={{ position: "relative", width: 400, height: 400, border: "1px solid black" }}
           onMouseMove={move}
           onMouseLeave={leave}>
        <Cursor position={sharedCursor} />
      </div>
    </VerticalLayout>
  )
}
const cursorImg = "data:;base64,";

const Cursor = memo(function Cursor({position}: {position: ValueSignal<Position | undefined>}) {
  if (!position.value) {
    return <></>;
  }
  const { x: left, y: top} = position.value;
  const style = {
    filter: 'invert(10%) sepia(100%) saturate(10000%) hue-rotate(0deg) brightness(100%) contrast(100%)',
    position: "absolute", left, top
  };
  return <img src={cursorImg} style={ style } />;
});